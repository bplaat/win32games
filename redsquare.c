...
#include <windows.h>
HDC hdc; HBRUSH background, white, red, blue; 
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  switch (msg) {
    case WM_CREATE:
      srand(getpid()); hdc = GetDC(hwnd), white = CreateSolidBrush(RGB(255, 255, 255)), red = CreateSolidBrush(RGB(255, 0, 0)), blue = CreateSolidBrush(RGB(0, 0, 255));
    ...
    case WM_PAINT:
      SelectObject(hdc, background); Rectangle(hdc, 0, 0, w, h);
      SelectObject(hdc, white); Rectangle(hdc, 50, 50, w - 50, h - 50);
      sprintf(str, "Score: %06d  -  Time: %03ds  -  Level: %02d", score, time / 40, level);
      TextOut(hdc, 50, 5, (LPCTSTR)&str, 42);
