...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  switch (msg) {
    ...
    case WM_PAINT: {
      ...
      SelectObject(hdc, CreateSolidBrush(RGB(0, 0, 175)));
      Rectangle(hdc, 0, 0, W, H);
      
      for (i = 0; i < MW; i++) for (j = 0; j < MH-i%2; j++) {
        if (*(map+i*MH+j) == 0) SelectObject(hdc, CreateSolidBrush(RGB(0, 0, 175)));
        if (*(map+i*MH+j) == 1) SelectObject(hdc, CreateSolidBrush(RGB(0, 175, 0)));
        if (*(map+i*MH+j) == 2) SelectObject(hdc, CreateSolidBrush(RGB(255, 255, 255)));
      ...
      TextOut(hdc, 20, H - 60, (LPCTSTR)&turn_str, 13);
      sprintf(&ws_str, "World size: %3d", world_size);
      TextOut(hdc, 20, H - 40, (LPCTSTR)&ws_str, 15);
      EndPaint(hwnd, &ps);
    } break;
